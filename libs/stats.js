    ...
    async.each(
          ...
          if (err) {
            ...
            for (var i = 0; i < replies.length; i += commandsPerCoin) {
              ...
              let [blocks, fields] = await connection.execute(
                `SELECT 
                  height, confirmations, time, difficulty  
                  FROM blocks WHERE coinid = ? ORDER BY height DESC LIMIT 50`,
                [coin_id],
              );
              let [blockStats, fields2] = await connection.execute(
                `SELECT 
                    SUM(case when confirmations > 20 then 1 else 0 end) AS validBlocks
                  FROM blocks WHERE coinid = ? ORDER BY height`,
                [coin_id],
              );
              //console.log('block stats', blockStats);
              // let formattedBlocks = [];
              // blocks.forEach(block => {
              //   formattedBlocks.push([block.confirmations, block.difficulty, block.height, '', block.time].join(':'));
              // });
              ...
              var coinStats = {
                ...
                blocks: {
                  pending: 3, //replies[i + 3],
                  confirmed: 4, //replies[i + 4],
                  orphaned: 5, //replies[i + 5],
                },
      ...
      function(err) {
        ...
        Object.keys(allCoinStats).forEach(function(coin) {
          ...
          var shareMultiplier = 480000;//Math.pow(2, 32) / algos[coinStats.algorithm].multiplier;
